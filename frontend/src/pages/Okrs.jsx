...
const Okrs = () => {
  ...
  const fetchData = async () => {
    try {
      const [okrsRes, okrTypesRes, employeesRes, ownersRes, userRes] = await Promise.all([
        getOkrs(),
        getOkrTypes(),
        getEmployees(),
        getAvailableOwners(),
  ...
  const handleSubmitCheckIn = async (e) => {
    ...
    try {
      ...
      formData.append('current_value', parseFloat(checkInFormData.current_value) || 0)
  ...
  const canApproveOrReject = (checkIn) => {
    ...
    const approverId = checkIn.objective?.approver || selectedObjective?.approver
    const isAdmin = currentUser.role?.name === 'Admin'
    const isDesignatedApprover = approverId === currentUser.id
    const canApprove = (isAdmin || isDesignatedApprover) && checkIn.current_status === 'pending'
    return canApprove
  }
  ...
  const filteredOkrs = okrs.filter(okr => {
    const matchesSearch = okr.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gradient-to-r from-blue-50 to-red-50">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">OKR Name</th>
              ...
              {filteredOkrs.length === 0 ? (
                <tr>
                  <td colSpan="8" className="px-6 py-12 text-center text-gray-500">
                    ...
                    <p className="text-sm mt-1">Create a new OKR to get started</p>
                  </td>
                </tr>
              ) : (
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">OKR Name *</label>
                  <input
                    type="text"
                    required
                    value={formData.name}
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">OKR Type *</label>
                  <select
                    required
                    value={formData.okr_type_id}
                    onChange={(e) => {
                      const selectedType = okrTypes.find(t => t.id === parseInt(e.target.value))
                      setFormData({
                        ...formData,
                        okr_type_id: e.target.value,
                        owner_type: selectedType?.is_employee ? 'App\\Models\\Employee' : 'App\\Models\\OrgUnit',
                        owner_id: ''
                      })
                    }}
                    className="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-primary focus:ring-4 focus:ring-primary/20 transition-all duration-200 outline-none"
                  ...
                  {okrTypes.length === 0 && (
                    <p className="text-xs text-red-500 mt-1">No OKR types available. Please create OKR types first.</p>
                  )}
                </div>
                <div>
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">
                    {formData.owner_type === 'App\\Models\\Employee' ? 'Employee *' : 'Organization Unit *'}
                  </label>
                  <select
                    required
                  ...
                  >
                    ...
                    {formData.owner_type === 'App\\Models\\Employee' ? (
                      availableOwners?.employees?.map((emp) => (
                        <option key={emp.id} value={emp.id}>{emp.title || emp.name}</option>
                      ))
                    ) : (
                      availableOwners?.org_units?.map((org) => (
                        <option key={org.id} value={org.id}>{org.title || org.name}</option>
                      ))
                    )}
                  </select>
                </div>
              ...
              <div className="border-t border-gray-200 pt-4">
                ...
                {formData.objectives.length === 0 ? (
                  <div className="text-center py-8 bg-gray-50 rounded-xl">
                    <p className="text-gray-500">No objectives added yet. Click "Add Objective" to create one.</p>
                  </div>
                ) : (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                          <div className="md:col-span-3">
                            ...
                            <input
                              ...
                              onChange={(e) => updateObjectiveInForm(index, 'description', e.target.value)}
                          ...
                          <div>
                            <label className="block text-xs font-semibold text-gray-700 mb-1">Target Type *</label>
                            <select
                              value={objective.target_type}
                              onChange={(e) => updateObjectiveInForm(index, 'target_type', e.target.value)}
                              className="w-full px-3 py-2 rounded-lg border-2 border-gray-200 focus:border-primary focus:ring-4 focus:ring-primary/20 transition-all duration-200 outline-none text-sm"
                            >
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => updateObjectiveInForm(index, 'target_value', e.target.value)}
                              placeholder="100"
                              className="w-full px-3 py-2 rounded-lg border-2 border-gray-200 focus:border-primary focus:ring-4 focus:ring-primary/20 transition-all duration-200 outline-none text-sm"
                            />
                          </div>
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => updateObjectiveInForm(index, 'deadline', e.target.value)}
                              className="w-full px-3 py-2 rounded-lg border-2 border-gray-200 focus:border-primary focus:ring-4 focus:ring-primary/20 transition-all duration-200 outline-none text-sm"
                            />
                          </div>
                          <div>
                            <label className="block text-xs font-semibold text-gray-700 mb-1">Tracking Type *</label>
                            <select
                              value={objective.tracking_type}
                              onChange={(e) => updateObjectiveInForm(index, 'tracking_type', e.target.value)}
                              className="w-full px-3 py-2 rounded-lg border-2 border-gray-200 focus:border-primary focus:ring-4 focus:ring-primary/20 transition-all duration-200 outline-none text-sm"
                            >
                          ...
                          <div>
                            <label className="block text-xs font-semibold text-gray-700 mb-1">Tracker *</label>
                            <select
                              value={objective.tracker}
                              onChange={(e) => updateObjectiveInForm(index, 'tracker', e.target.value)}
                              className="w-full px-3 py-2 rounded-lg border-2 border-gray-200 focus:border-primary focus:ring-4 focus:ring-primary/20 transition-all duration-200 outline-none text-sm"
                            >
                          ...
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) => updateObjectiveInForm(index, 'approver', e.target.value)}
                              className="w-full px-3 py-2 rounded-lg border-2 border-gray-200 focus:border-primary focus:ring-4 focus:ring-primary/20 transition-all duration-200 outline-none text-sm"
                            >
                              <option value="">Select Employee</option>
                              {employees.map((emp) => (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
            <div className="p-6 border-b border-gray-200 flex items-center justify-between">
              <div>
                <h2 className="text-xl text-gray-900"><span className='font-bold'>{selectedOkr.name}</span> - {selectedOkr.owner.name}</h2>
                <p className="text-sm text-gray-500">
                  {new Date(selectedOkr.start_date).toLocaleDateString("en-GB", {
  day: "numeric",
  month: "short",
  year: "numeric",
  ...
  year: "numeric",
              ...
              <button
                onClick={() => { setShowObjectivesModal(false); setSelectedOkr(null) }}
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {objectives.length === 0 ? (
                <div className="text-center py-12 text-gray-500">
                  ...
                  <p className="text-sm mt-1">Create objectives when creating or editing an OKR</p>
                </div>
              ) : (
                      ...
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-gray-600">
                            <div>
                              <span className="font-medium">Target:</span> {objective.target_value} ({objective.target_type})
                            </div>
                            <div>
                              <span className="font-medium">Deadline:</span> {new Date(objective.deadline).toLocaleDateString()}
                            </div>
                            <div>
                              <span className="font-medium">Tracker:</span> {objective.tracker_employee?.name || 'N/A'}
                            </div>
                            <div>
                              <span className="font-medium">Approver:</span> {objective.approver_employee?.name || 'N/A'}
                            </div>
                            <div>
                              <span className="font-medium">Tracking:</span> {objective.tracking_type}
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center gap-2 ml-4">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
            <div className="p-6 border-b border-gray-200 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">{selectedObjective.description}</p>
              </div>
              <button
                onClick={() => { setShowCheckInModal(false); setSelectedObjective(null); resetCheckInForm() }}
                className="p-2 hover:bg-gray-100 rounded-lg transition-colors"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              <div className="p-6 border-b border-gray-200 bg-gray-50">
                <h3 className="text-sm font-semibold text-gray-700 mb-3">Add New Check-In</h3>
                <form onSubmit={handleSubmitCheckIn} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setCheckInFormData({ ...checkInFormData, current_value: e.target.value })}
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => setCheckInFormData({ ...checkInFormData, comments: e.target.value })}
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <label className="flex-1">
                        ...
                        <div className="w-full px-3 py-2 rounded-lg border-2 border-gray-200 cursor-pointer hover:border-primary transition-colors">
                          {evidenceFileName ? (
                            <span className="text-sm text-gray-700">{evidenceFileName}</span>
                          ) : (
                      ...
                      {evidenceFileName && (
                        <button
                          type="button"
                          onClick={() => { setCheckInFormData({ ...checkInFormData, evidence_file: null }); setEvidenceFileName('') }}
                    ...
                    <p className="text-xs text-gray-500 mt-1">Accepted: PDF, DOC, DOCX, JPG, PNG, XLS, XLSX</p>
                  </div>
              ...
              {/* CheckIns History */}
              <div className="p-6">
                <h3 className="text-sm font-semibold text-gray-700 mb-3">Check-In History</h3>
                {checkIns.length === 0 ? (
                  <div className="text-center py-8 text-gray-500">
                    <svg className="mx-auto h-10 w-10 text-gray-400 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                    </svg>
                    <p className="text-sm">No check-ins yet. Add your first check-in above.</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {checkIns.map((checkIn) => (
                      <div key={checkIn.id} className="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm">
                            ...
                            <div className="flex items-center gap-3 flex-1">
                              ...
                              <div className="flex-1">
                                <div className="flex items-center gap-2 flex-wrap">
                                  <p className="font-semibold text-gray-900">{checkIn.current_value} / {selectedObjective.target_value}</p>
                                  {checkIn.current_status && (
                                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${getStatusBadge(checkIn.current_status)}`}>
                                      {checkIn.current_status.charAt(0).toUpperCase() + checkIn.current_status.slice(1)}
                                    </span>
                                  )}
                        ...
                        {/* Approval Logs */}
                        {approvalLogs[checkIn.id] && approvalLogs[checkIn.id].length > 0 && (
                          <div className="px-4 py-3 bg-gray-50 border-t border-gray-200">
                            <h4 className="text-xs font-semibold text-gray-700 mb-2">Approval History</h4>
                              ...
                              {approvalLogs[checkIn.id].map((log) => (
                                <div key={log.id} className="flex items-center gap-2 text-xs">
                                  ...
                                  <span className={`font-medium ${
                                    log.status === 'approved' ? 'text-green-700' :
                                    log.status === 'rejected' ? 'text-red-700' :
                                    'text-yellow-700'
                                  }`}>
