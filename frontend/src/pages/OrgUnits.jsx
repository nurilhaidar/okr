...
const OrgUnits = () => {
  ...
  const handleUpdateMemberRole = async (memberId, newRoleId) => {
    try {
      await updateOrgUnitMemberRole(selectedOrgUnit.id, memberId, {
        orgunit_role_id: newRoleId,
      });
      await fetchMembers(selectedOrgUnit.id);
      await fetchAvailableRoles(selectedOrgUnit.id);
    } catch (error) {
  ...
  const buildOrgUnitTree = () => {
    // Get all root org units (no parent)
    const rootOrgUnits = orgUnits.filter((ou) => !ou.parent_id);

    // Recursive function to get children
    const getChildren = (parentId) => {
      return orgUnits.filter((ou) => ou.parent_id === parentId);
    };

    return rootOrgUnits.map((orgUnit) => ({
      ...orgUnit,
      children: buildTreeRecursive(orgUnit.id, getChildren),
    }));
  };

  const buildTreeRecursive = (parentId, getChildrenFn) => {
    const children = getChildrenFn(parentId);
    return children.map((child) => ({
      ...child,
      children: buildTreeRecursive(child.id, getChildrenFn),
    }));
  };

  // Recursive render for nested org units
  const renderOrgUnitNode = (orgUnit, level = 0) => {
    const hasChildren = orgUnit.children && orgUnit.children.length > 0;

    return (
      <div key={orgUnit.id} className="relative">
        {/* OrgUnit Row */}
        <div
          className={`flex items-center py-3 px-4 hover:bg-blue-50/50 transition-colors border-l-2}`}
        ...
        >
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              className="p-1.5 text-accent hover:bg-red-100 rounded-lg transition-colors"
              title="Delete"
            >
              <svg
                className="w-4 h-4"
        ...
        {/* Children (nested) - always visible */}
        {hasChildren && (
          <div>
            {orgUnit.children.map((child, index) => {
              const isLastChild = index === orgUnit.children.length - 1;
              return (
                <div key={child.id} className="relative">
                  {renderOrgUnitNode(child, level + 1, isLastChild)}
                </div>
              );
            })}
          </div>
  ...
  // Filter org units based on search and status
  const filterOrgUnits = (orgUnitsList, searchTerm, statusFilter) => {
    return orgUnitsList
      .filter((orgUnit) => {
        const matchesSearch =
          !searchTerm ||
          orgUnit.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
          (orgUnit.custom_type &&
            orgUnit.custom_type
              .toLowerCase()
        ...
        // Also check if any descendants match
        const descendantsMatch =
          orgUnit.children &&
          orgUnit.children.length > 0 &&
          filterOrgUnits(orgUnit.children, searchTerm, statusFilter).length > 0;

        const includeNode = matchesSearch && matchesStatus;

        // Recursively filter children
        const filteredChildren = orgUnit.children
          ? filterOrgUnits(orgUnit.children, searchTerm, statusFilter)
          : [];

        // Include this node if it matches or if any filtered children exist
  ...
  const treeOrgUnits = buildOrgUnitTree();
  const filteredTreeOrgUnits = filterOrgUnits(
    treeOrgUnits,
    searchTerm,
    statusFilter,
  );

  const totalCount = orgUnits.filter((ou) => {
    const matchesStatus =
      statusFilter === "all" ||
      (statusFilter === "active" && ou.is_active) ||
      (statusFilter === "inactive" && !ou.is_active);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Manage organizational units (Tree View)
          </p>
        </div>
        <button
          onClick={() => {
      ...
      <div className="bg-white rounded-2xl shadow-sm border-2 border-gray-100 overflow-hidden">
        {filteredTreeOrgUnits.length === 0 ? (
          ...
          <div className="divide-y divide-gray-200">
            {filteredTreeOrgUnits.map((orgUnit) => renderOrgUnitNode(orgUnit))}
          </div>
        )}
      </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      orgunit_type_id: e.target.value,
                    })
                  }
                  className="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-primary focus:ring-4 focus:ring-primary/20 transition-all duration-200 outline-none"
                >
              ...
              <div>
                ...
                >
                  ...
                  {orgUnits
                    .filter(
                      (ou) => !editingOrgUnit || ou.id !== editingOrgUnit.id,
                    )
                    .map((ou) => (
                          ...
                          >
                            {availableRoles.map((role) => (
                              <option
                                ...
                                disabled={
                                  !role.available &&
                                  role.id !== member.orgunit_role_id
                                }
                              >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setMemberFormData({
                      ...memberFormData,
                      orgunit_role_id: e.target.value,
                    })
                  }
                  required
                  className="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-primary focus:ring-4 focus:ring-primary/20 transition-all duration-200 outline-none"
                ...
                {!availableRoles.filter((role) => role.available).length && (
                  <p className="text-xs text-amber-600 mt-1">
                    All exclusive roles are taken. Only Member role is
                    available.
                  </p>
                )}
              </div>
