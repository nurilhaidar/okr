        ...
        <div class="card-body">
          <div class="d-flex justify-content-between align-items-center">
            <div>
              ...
              <p class="text-muted mb-0">Manage your organization's structure and hierarchy.</p>
            </div>
            <a href="{{ route('admin.org-units.create') }}" class="btn btn-primary">
              <i class="ti ti-building-plus me-2"></i>Add Unit
            </a>
          </div>
        </div>
      </div>
              ...
              <tbody>
                @forelse($orgUnitsTree as $orgUnit)
                  <tr>
                    ...
                    <td>
                      <span class="badge bg-label-info">{{ $orgUnit->members_count ?? 0 }} member{{ ($orgUnit->members_count ?? 0) != 1 ? 's' : '' }}</span>
                    </td>
                    <td>
                    ...
                    <td>
                      <div class="d-flex gap-1">
                        <button type="button" class="btn btn-sm btn-outline-info" onclick="openViewStructureModal({{ $orgUnit->id }}, '{{ $orgUnit->name }}')" title="View Structure">
                          <i class="ti ti-users"></i>
                        </button>
                        <a href="{{ route('admin.org-units.edit', $orgUnit->id) }}" class="btn btn-sm btn-outline-primary" title="Edit">
                          <i class="ti ti-pencil"></i>
                        </a>
                        @if($orgUnit->is_active)
                          <form method="POST" action="{{ route('admin.org-units.deactivate', $orgUnit->id) }}" class="d-inline">
                            @csrf
                            <button type="submit" class="btn btn-sm btn-outline-warning" title="Deactivate" onclick="return confirm('Are you sure you want to deactivate this org unit?')">
                              <i class="ti ti-player-pause"></i>
                            </button>
                          </form>
                        @else
                          <form method="POST" action="{{ route('admin.org-units.activate', $orgUnit->id) }}" class="d-inline">
                            @csrf
                        ...
                        @endif
                        <form method="POST" action="{{ route('admin.org-units.destroy', $orgUnit->id) }}" class="d-inline">
                          @csrf
                          @method('DELETE')
                          <button type="submit" class="btn btn-sm btn-outline-danger" title="Delete" onclick="return confirm('Are you sure you want to delete this org unit?')">
